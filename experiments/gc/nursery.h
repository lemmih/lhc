...
#define allocate(ns, semi, tag, ...) _allocate(ns, semi, tag, ((Object)(__VA_ARGS__)), sizeof((__VA_ARGS__)))
// hp _allocate(Nursery *ns, enum Tag t, Object o, unsigned long size);

void nursery_evacuate(Nursery *ns, SemiSpace *semi, hp* objAddr);
void nursery_reset(Nursery *ns, Stats *);
void nursery_bypass(Nursery *ns, SemiSpace *semi);
bool nursery_member(Nursery *ns, hp obj);

static hp _allocate(Nursery *ns, SemiSpace *semi, enum Tag t, Object o, unsigned long size) {
  hp ret = ns->index;
  Header header;
